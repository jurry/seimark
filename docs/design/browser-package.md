...
`attach` throws synchronously, before any frame flows, for conditions settled up front: no encoded transform in the browser (`unsupported_browser`), a stream id whose length is not 8 or a sender with no track (`invalid_argument`), a sender already attached and not since detached (`already_attached`), and a blob worker that the page's content security policy refuses (`csp_blocked`). These four are the integration's own codes and are distinct from the codec's, so a caller switching on `code` can tell a setup mistake from a bad frame. The last one is a real deployment failure — a policy without `worker-src blob:` blocks the worker the package builds — so it is detected at attach and reported as a policy error naming the directive, not left as a silent stream that never marks.

`detach` differs by path. On the standard path the transform is removed from the sender and the sender becomes attachable again, which is what a reconnect that keeps its stream id needs: `docs/format.md` has an application continuing a sequence across a reconnect, and that is unreachable if the first `detach` retires the sender for good. On the fallback, `createEncodedStreams` can be called once per sender and the pipe cannot be undone, so `detach` puts the transform into pass-through: frames continue to flow untouched, the handle stops updating, and the sender stays refused for the life of the page. The asymmetry is documented rather than hidden, because a page that detaches and expects the pipeline gone would otherwise be surprised.
...
- **`attach`'s synchronous throws, against a fake worker.** The four setup codes and the re-attach after `detach` are driven with a stub `RTCRtpScriptTransform` and `Worker`, so the conditions settled before any media flows are covered without a peer connection.
